...
function showTaskInput(data) {
  const container =
    document.getElementById("no-show") || document.getElementById("yes-show");
...
function createTaskForm(data) {
  ...
  const descriptionWrapper = createInputWrapper(
    "Description:",
    "text",
    "description",
    true,
  ...
  const favouriteWrapper = createInputWrapper(
    "Favourite:",
    "checkbox",
    "favourite",
    false,
  );
  const importantWrapper = createInputWrapper(
    "Important:",
    "checkbox",
    "important",
    false,
...
function printMain(event) {
  ...
  if (event.target.classList[0] === "time-period") {
    ...
    otherDivs.forEach((otherDiv) =>
      otherDiv.classList.remove("clicked-project"),
    );
